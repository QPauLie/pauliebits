...
When creating a new pauliebits object, the endianness can always be
specified explicitly:

.. code-block:: python

    >>> a = pauliebits(b'A', endian='little')
    >>> a
    pauliebits('10000010')
    >>> a.endian
    'little'
...
The bit-endianness is a property of the pauliebits object.
The endianness cannot be changed once a pauliebits object has been created.
When comparing pauliebits objects, the endianness (and hence the machine
representation) is irrelevant; what matters is the mapping from indices
to bits:

.. code-block:: python

    >>> pauliebits('11001', endian='big') == pauliebits('11001', endian='little')
    True
    >>> a = pauliebits(b'\x01', endian='little')
    >>> b = pauliebits(b'\x80', endian='big')
    >>> a == b
    True
    >>> a.tobytes() == b.tobytes()
    False
...
created.  However, you can create a new pauliebits with different endianness:

.. code-block:: python

    >>> a = pauliebits('111000', endian='little')
...
When explicitly requesting a little-endian pauliebits, we get:
...
the pauliebits it is provided with:

.. code-block:: python

    >>> a = pauliebits("11001", "little")
    >>> ba2int(a)
    19
    >>> ba2int(pauliebits(a, "big"))
    25
...
.. code-block:: python
    ...
    >>> from pauliebits.util import ba2hex, hex2ba, ba2base, base2ba
    >>> for endian in "little", "big":
    ...     a = pauliebits("1010 0011 1110", endian)
