...
pauliebits's ``.decode()`` and ``.search()`` methods to return iterators.
This is similar to how Python's ``dict.keys()``, ``.values()``
and ``.items()`` methods were revamped in the Python 2 to 3 transition.

In the following table, ``a`` is assumed to be a pauliebits object.
...
Aside from these changes, which will make pauliebits 3 more pythonic, there
are a few other minor changes (see changelog).
It should be emphasized that in most common use cases the pauliebits 3
