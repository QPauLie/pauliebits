...
{
    PyObject *str_obj;
    /* Parse the input arguments to ensure a single Python string (Unicode object) is passed */
    if (!PyArg_ParseTuple(args, "U", &str_obj)) {
        return NULL;
    }
    /* Check if the current pauliebits instance buffer is read-only */
    if (self->readonly) {
        PyErr_SetString(PyExc_BufferError, "cannot resize read-only pauliebits");
        return NULL;
    }

    /* Retrieve the exact length of the input Unicode string */
    Py_ssize_t len = PyUnicode_GET_LENGTH(str_obj);
    /* Handle the boundary case of an empty string input */
    if (len == 0) {
        if (resize(self, 0) < 0) {
            return NULL;
        }
        /* Reset to default Big Endian format for the empty array */
        self->endian = 1; 
        Py_RETURN_NONE;
    }

    /* Each character requires 2 bits, calculate total bit length and corresponding byte count */
    Py_ssize_t nbits = len * 2;
    Py_ssize_t nbytes = (nbits + 7) / 8;

    /* Allocate a temporary heap buffer via Python's memory management API */
    ...
    /* --- Fast processing in 4-character blocks (compiles into 1 full byte) --- */
    Py_ssize_t fast_len = len - (len % 4); 
    for (; i < fast_len; i += 4) {
        char b0 = 0, b1 = 0, b2 = 0, b3 = 0;
        
        /* Direct read of 4 consecutive characters from the Python string object */
        ...
        /* Mapping: I=0 (00), Z=1 (01), X=2 (10), Y=3 (11) */
        b0 = (c0 == 'Z') ? 1 : (c0 == 'X') ? 2 : (c0 == 'Y') ? 3 : 0;
        b1 = (c1 == 'Z') ? 1 : (c1 == 'X') ? 2 : (c1 == 'Y') ? 3 : 0;
        b2 = (c2 == 'Z') ? 1 : (c2 == 'X') ? 2 : (c2 == 'Y') ? 3 : 0;
        b3 = (c3 == 'Z') ? 1 : (c3 == 'X') ? 2 : (c3 == 'Y') ? 3 : 0;

        /* Pack the 4 mapped 2-bit values into a single byte using Big Endian bit sequencing */
    ...
    for (; i < len; i++) {
        ...
        /* Mask the 2-bit value into the current byte at the appropriate bit offset */
        buffer[byte_idx] |= (val << bit_shift);
        bit_shift -= 2;
        /* Move to the next byte block once the current byte is full */
        if (bit_shift < 0) {
            bit_shift = 6;
            byte_idx++;
        }
    ...
    /* Resize the internal array storage to comfortably fit the newly computed number of bits */
    if (resize(self, nbits) < 0) {
        PyMem_Free(buffer);
        return NULL;
    }

    /* Explicitly enforce Big Endian flag to match the layout of the generated byte stream */
    self->endian = 1;

    /* Safely transfer the contents from the temporary memory chunk to the underlying object buffer */
    memcpy(self->ob_item, buffer, nbytes);
    PyMem_Free(buffer);

    /* Zero out any unused remainder bits in the very last byte to ensure clean object comparisons */
    if (nbits & 7) {
...
/* Cross-platform 64-bit population count (popcount) implementation.
   Chooses the fastest hardware instruction depending on the compiler. */
#if defined(_MSC_VER)
    /* Microsoft Visual Studio compiler */
    #include <intrin.h>
    /* Use the MSVC intrinsic for hardware-accelerated 64-bit bit counting */
    #define POPCOUNT64(x) __popcnt64(x)
#elif defined(__GNUC__) || defined(__clang__)
    /* GCC or Clang compilers */
    /* Use the built-in compiler function for hardware-accelerated 64-bit bit counting */
    #define POPCOUNT64(x) __builtin_popcountll(x)
#else
    /* Fallback software implementation if no hardware-accelerated intrinsic is available */
    static inline int POPCOUNT64(unsigned long long v) {
        /* Step 1: Count bits within every 2-bit chunk */
        v = v - ((v >> 1) & 0x5555555555555555ULL);
        /* Step 2: Sum adjacent 2-bit counts to get 4-bit chunk counts */
        v = (v & 0x3333333333333333ULL) + ((v >> 2) & 0x3333333333333333ULL);
        /* Step 3: Sum adjacent 4-bit counts into 8-bit byte totals, 
                  then multiply by a magic mask to accumulate all byte counts into the highest byte,
                  and finally shift right by 56 bits to extract the total sum */
...
{
    PyObject *other_obj;
    /* Parse and validate that the input argument matches the custom Pauliebits_Type */
    ...
    /* Determine the overlapping region length (minimum of both arrays) */
    Py_ssize_t min_bits = (self_bits < other_bits) ? self_bits : other_bits;
    Py_ssize_t nbytes = (min_bits + 7) / 8;

    /* If either array is empty, they trivially commute */
    if (nbytes == 0) {
        Py_RETURN_TRUE;
    }
    ...
    Py_ssize_t i = 0;
    /* Compute length for fast loop processing in chunks of 8 bytes (64-bit words) */
    Py_ssize_t fast_bytes = nbytes - (nbytes % 8);

    /* --- Fast loop processing 64 bits at a time --- */
    for (; i < fast_bytes; i += 8) {
        unsigned long long s_word;
        unsigned long long o_word;

        /* Safely extract 8 bytes into 64-bit unsigned integers to prevent unaligned access issues */
        memcpy(&s_word, self->ob_item + i, 8);
        memcpy(&o_word, other->ob_item + i, 8);

        /* Alternating bitmask 0101...0101 to isolate even bit positions */
        unsigned long long even_mask = 0x5555555555555555ULL;

        /* Intersect even bits of 'self' with right-shifted odd bits of 'other' */
        unsigned long long and_1 = (s_word & even_mask) & (o_word >> 1);
        total_ones_1 += POPCOUNT64(and_1);

        /* Intersect even bits of 'other' with right-shifted odd bits of 'self' */
        unsigned long long and_2 = (o_word & even_mask) & (s_word >> 1);
        total_ones_2 += POPCOUNT64(and_2);
    }

    /* --- Remainder loop processing byte-by-byte --- */
    for (; i < nbytes; i++) {
        unsigned char s_byte = (unsigned char)self->ob_item[i];
        unsigned char o_byte = (unsigned char)other->ob_item[i];

        /* Clean up the unaligned tail of the very last byte if nbits is not a multiple of 8 */
        if (i == nbytes - 1 && (min_bits % 8) != 0) {
            int extra_bits = 8 - (min_bits % 8);
            /* Mask tailored for Big-Endian alignment (default behavior of original pauliebits) */
            unsigned char tail_mask = 0xFF << extra_bits; 
            s_byte &= tail_mask;
            o_byte &= tail_mask;
        }

        /* Isolate and check intersections for the current byte */
        total_ones_1 += POPCOUNT64((s_byte & 0x55) & (o_byte >> 1));
        total_ones_2 += POPCOUNT64((o_byte & 0x55) & (s_byte >> 1));
    }

    /* Check if the parities of both total intersection counts match.
       If parities are equal, the Pauli operators commute. */
...
{
    /* Calculate the total number of bit pairs (even/odd combinations) */
    ...
    /* Define a helper macro to read a specific bit dynamically based on Big/Little Endian format */
    #ifndef GET_BIT
    ...
    /* Iterate sequentially through each logical bit pair */
    for (size_t i = 0; i < num_pairs; i++) {
        size_t even_idx = i * 2;
        size_t odd_idx = i * 2 + 1;

        /* Extract the even and odd bit values (normalized to 0 or 1) from the underlying buffer */
        int even_bit = GET_BIT(self->ob_item, even_idx, self->endian) ? 1 : 0;
        int odd_bit  = GET_BIT(self->ob_item, odd_idx, self->endian) ? 1 : 0;

        /* Check the OR condition for the pair; increment the counter if at least one bit is set to 1 */
        if (even_bit | odd_bit) {
            total_count++;
        }
    }

    /* Return the aggregated count back to Python as a standard size_t integer object */
...
{
    /* Return 0 as a Python integer if the input array is completely empty */
    if (self->nbits == 0) {
        return PyLong_FromLong(0);
    }

    /* Calculate the total amount of logical bit pairs available in the array */
    size_t num_pairs = (size_t)(self->nbits / 2);

    /* Define a helper macro to extract a single bit from the buffer based on Endianness rules */
    #ifndef GET_BIT
    #define GET_BIT(ptr, index, endian) \
        ((endian == 1) ? \
         (((const unsigned char *)(ptr))[(index) >> 3] & (0x80 >> ((index) & 7))) : \
    ...
    /* Step 1: Scan strictly through logical even positions (index i * 2).
       If any even bit is found to be 1, immediately abort and return -1. */
    for (size_t i = 0; i < num_pairs; i++) {
    ...
    /* Safety fallback check to prevent zero allocation bounds */
    if (num_pairs == 0) {
        return PyLong_FromLong(0);
    }

    /* Compute the necessary byte capacity required to house the newly formed integer */
    size_t res_bytes = (num_pairs + 7) >> 3;
    /* Dynamically allocate a clean memory region using Python's specific memory framework */
    ...
    /* Step 2: Extract all valid odd bits and construct the intermediate byte representation.
       Since Big-Endian arrays treat index 0 as the most significant bit of the integer,
       the target position within the byte stream is mapped in reverse via 'num_pairs - 1 - i'. */
    for (size_t i = 0; i < num_pairs; i++) {
    ...
    /* Step 3: Deserialize the raw byte chunk into a fully fledged Python integer object.
       The parsing parameters (1, 0) configure it for Little Endian bytes and Unsigned interpretation. */
    PyObject *result = _PyLong_FromByteArray(res_buffer, res_bytes, 
                                            1, 
                                            0  );

    /* Safely release the transient memory buffer block to eliminate memory leak vectors */
...
{
    /* Type verification: ensure the passed argument matches the custom structure size.
       This guards against type mismatches while supporting dynamic pytest wrappers. */
    ...
    /* Dimensionality check: both pauliebits sequences must contain the exact same bit count */
    if (self->nbits != other->nbits) {
        PyErr_SetString(PyExc_ValueError, "pauliebits objects must have the same length");
        return NULL;
    }

    /* Fast return if the bit arrays are empty */
    if (self->nbits == 0) {
        return PyLong_FromLong(0);
    }
    ...
    /* Accumulators for counting intersections (AND operations) between different bit slices */
    ...
    /* Total number of logical bit pairs available in the array */
    size_t res_bits = (size_t)(self->nbits / 2);

    /* Main loop: sequentially iterate through each logical bit pair */
    for (size_t i = 0; i < res_bits; i++) {
        size_t even_idx = i * 2;
        size_t odd_idx = i * 2 + 1;

        /* Extract even and odd bit values for the 'self' instance */
        int s_even = GET_BIT(self->ob_item, even_idx, self->endian) ? 1 : 0;
        int s_odd  = GET_BIT(self->ob_item, odd_idx, self->endian) ? 1 : 0;

        /* Extract even and odd bit values for the 'other' instance */
        int o_even = GET_BIT(other->ob_item, even_idx, other->endian) ? 1 : 0;
        int o_odd  = GET_BIT(other->ob_item, odd_idx, other->endian) ? 1 : 0;

        /* Increment the respective counters based on specific bitwise intersection logic */
    ...
    /* Compute the final value using the specific quantum phase evaluation formula */
    long long final_result = (2 * (long long)sum_1) + (long long)sum_2 + (long long)sum_3 - (long long)sum_4;

    /* Return the signed total back to Python as a standard LongLong object */
...
{
    /* Calculate the total number of logical bit pairs available in the array */
    size_t num_pairs = (size_t)(self->nbits / 2);
    /* Fast return if the bit array has no pairs to process */
    if (num_pairs == 0) {
        return PyLong_FromLong(0);
    }
    ...
    for (size_t i = 0; i < num_pairs; i++) {
        ...
        /* Check the AND condition for the pair; increment the counter if both bits are set to 1.
           In Pauli mapping, this corresponds to finding the number of 'Y' operators. */
        if (even_bit & odd_bit) {
            total_ys++;
        }
    }
...
{
    /* Calculate the total number of bits required for the compressed output array */
    size_t res_bits = (size_t)(self->nbits / 2);
    
    /* Step 1: Allocate memory for the new pauliebits object using Python's specific type allocator.
       This automatically zeroes out the base Python reference counter headers. */
    pauliebitsobject *res = (pauliebitsobject *)Py_TYPE(self)->tp_alloc(Py_TYPE(self), 0);
    if (res == NULL) {
        return NULL;
    }

    /* Initialize all structural and tracking fields to safe default states before layout adjustments */
    res->nbits = 0;
    res->ob_item = NULL;
    res->allocated = 0;
    res->endian = 1; /* Enforce Big Endian format to mirror standard Python slice operations */
    res->ob_exports = 0;
    res->weakreflist = NULL;
    res->buffer = NULL;
    res->readonly = 0;

    /* If the target bit count evaluates to zero, immediately hand back the empty instance */
    if (res_bits == 0) {
        return (PyObject *)res;
    }

    /* Step 2: Invoke the internal library sizing routine to construct the bit data buffer storage.
       This automatically manages systemic alignment variables like res->allocated. */
    if (resize(res, (Py_ssize_t)res_bits) < 0) {
        Py_DECREF(res);
        return NULL;
    }

    /* Flush out the newly managed data store bytes to maintain deterministic zero state initialization */
    size_t res_bytes = (size_t)res->allocated;
    memset(res->ob_item, 0, res_bytes);

    /* Helper macro to extract a targeted single bit based on explicit Endian configurations */
    #ifndef GET_BIT
    #define GET_BIT(ptr, index, endian) \
        ((endian == 1) ? \
         (((const unsigned char *)(ptr))[(index) >> 3] & (0x80 >> ((index) & 7))) : \
    ...
    /* Helper macro to safely assert a bit to 1 based on explicit Endian configurations */
    #ifndef SET_BIT_ON
    ...
    /* Step 3: Iterate across logical pairs to perform the compression operation via bitwise OR */
    for (size_t i = 0; i < res_bits; i++) {
        size_t even_idx = i * 2;
        size_t odd_idx = i * 2 + 1;

        /* Extract corresponding bit state pairs from the source layout using its native Endian flag */
        int even_bit = GET_BIT(self->ob_item, even_idx, self->endian) ? 1 : 0;
        int odd_bit  = GET_BIT(self->ob_item, odd_idx, self->endian) ? 1 : 0;

        /* Assert the destination bit to 1 if either operand evaluates to a true bit state */
        if (even_bit | odd_bit) {
            SET_BIT_ON(res->ob_item, i, 1);
        }
    }

    /* Step 4: Clear out any unaligned remainder trailing bits within the last active byte block.
       This ensures complete bitwise cleanliness for object comparison workflows. */
    if (res_bits & 7) {
...
{
    /* Calculate the total number of characters in the resulting Pauli string (1 char per bit pair) */
    size_t length = (size_t)(self->nbits / 2);
    if (length == 0) {
        return PyUnicode_FromString("");
    }

    /* Allocate a new Python string object. Since it only contains ASCII characters (I, X, Y, Z),
       we use max_char=127 to optimize it as a 1-byte kind string (PyUnicode_1BYTE_KIND) */
    PyObject *res_string = PyUnicode_New(length, 127);
    if (res_string == NULL) {
        return NULL;
    }
    
    /* Get a direct raw pointer to the underlying 1-byte character buffer of the Python string */
    Py_UCS1 *str_buf = PyUnicode_1BYTE_DATA(res_string);
    char *buffer = self->ob_item;

    /* Mapping array based on DECODEC rules where lookup index is formed as: (odd_bit << 1) | even_bit
    ...
    /* Main loop: sequentially iterate through each logical bit pair to reconstruct characters */
    for (size_t i = 0; i < length; i++) {
        size_t even_idx = i * 2;
        size_t odd_idx = i * 2 + 1;

        /* Extract even and odd bit states (normalized to 0 or 1) using the native object Endianness flag */
        uint8_t even_bit = GET_BIT(buffer, even_idx, self->endian) ? 1 : 0;
        uint8_t odd_bit  = GET_BIT(buffer, odd_idx, self->endian) ? 1 : 0;

        /* Combine the bits into a 2-bit lookup integer index (ranges from 0 to 3) */
        uint8_t lookup_idx = (odd_bit << 1) | even_bit;

        /* Write the mapped ASCII character directly into the allocated Python string buffer */
